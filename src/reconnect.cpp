...
void Tangle::Reconnection(){
	...
	if(recon_count!=0){
		while(recon_count > 0){
			...
			mLog << ctime(&(time(0)).substr(10,8) << "\t" << mStep << ": number of possible reconnections = " << recon_count << endl;
							...
							for(int l(0); l<mTangle[Q]->mN; l++){
								...
								if(mTangle[P]->mPoints[k]==mTangle[Q]->mPoints[l]
									...
									if(dist2 < 0.25*res*res){
										...
										if(dot_tangents > 0){mLog << ctime(&(time(0)).substr(10,8) << "\t" << mStep << ": ignoring parallel filaments" << endl; continue;}
			...
			while(i<mTangle[P]->mN){
				...
				if(NeedRecon==true){
					if(mTangle[P]->mPoints[l_rec]->mMarkedForRecon == true
						...
						mLog << ctime(&(time(0)).substr(10,8) << "\t" << mStep << ": attempting to reconnect at " << k << ", " << l_rec << endl;
					 	recon_count++;
					}
				}
				i++; pK = pK->mNext;
...
void Tangle::SelfReconnectLine(int P, int Q, int k, int l){
 		mLog << ctime(&(time(0)).substr(10,8) << "\t" << mStep << ": attempting line reconnection" << endl;
    mN_f = 1; mN_slow = 0;
    ...
    mLog << ctime(&(time(0)).substr(10,8) << "\t " << mStep << " : successful line reconnection" << endl;
}

void Tangle::SelfReconnect(int P, int Q, int k, int l){
	mLog << ctime(&(time(0)).substr(10,8) << "\t" << mStep << ": attempting self-reconnection" << endl;
	mN_f = 1; mN_slow = 0;
	/* reassign pointers to separate new ring  */
	mTangle[P]->mPoints[k]->mNext->mPrev = mTangle[Q]->mPoints[l]->mPrev;  
	mTangle[Q]->mPoints[l]->mPrev->mNext = mTangle[P]->mPoints[k]->mNext;
	...
	mTangle.erase(mTangle.begin()+P);
	cout << "\n\t\t !!! RECONNECTION !!! " << endl;
	mLog << ctime(&(time(0)).substr(10,8) << "\t" << mStep << ": successful self-reconnection" << endl;
}

void Tangle::Reconnect(int P, int Q, int k, int l){
	mLog << ctime(&(time(0)).substr(10,8) << "\t" << mStep << ": attempting reconnection" << endl;
	mN_f = 1; mN_slow = 0;
	...
	mLog << ctime(&(time(0)).substr(10,8) << "\t" << mStep << ": successful reconnection" << endl;
}