function [xres,yres,zres] = CalcSDoublePrime(x,y,z,ell)
...
res=2.*(-2.*l.*lp1+lp1.*lp1-l.*lp2+lp1.*lp2)./(lm1.*(lm1+l).*(lm1+l+lp1).*(lm1+l+lp1+lp2));
end

function res=CalcB(lm1,l,lp1,lp2)
res=2.*(2.*lm1.*lp1+2.*l.*lp1-lp1.*lp1+lm1.*lp2+l.*lp2-lp1.*lp2)./(lm1.*l.*(l+lp1).*(l+lp1+lp2));
end

function res=CalcD(lm1,l,lp1,lp2)
res=2.*(-lm1.*l-l.*l+lm1.*lp1+2.*l.*lp1+lm1.*lp2+2.*l.*lp2)./(lp1.*lp2.*(l+lp1).*(lm1+l+lp1));
end

function res=CalcE(lm1,l,lp1,lp2)
res=2.*(lm1.*l+l.*l-lm1.*lp1-2.*l.*lp1)./(lp2.*(lp1+lp2).*(l+lp1+lp2).*(lm1+l+lp1+lp2));
end
